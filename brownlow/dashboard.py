...
# (backtest_2026.py), never by weekly_update.py. A plain static table, not a
# sortable/toggleable one like the leaderboard: it's a single post-season
# snapshot, not a live-updating view, so that interactivity would be unused
# complexity.
...
<style>
  ...
  * {{ box-sizing: border-box; }}
  ...
  tbody td {{ padding: 11px 16px; border-bottom: 1px solid var(--row-line); vertical-align: middle; font-size: 14px; }}
  tbody tr:last-child td {{ border-bottom: none; }}
  tbody tr:hover td {{ background: rgba(240, 165, 0, 0.05); }}
  th.num, td.num {{ text-align: right; font-variant-numeric: tabular-nums; }}
  .rank {{ color: var(--gold); font-weight: 800; font-variant-numeric: tabular-nums; width: 40px; text-align: center; border-left: 3px solid transparent; }}
  .team-cell {{ }}
  .team-inner {{ display: flex; align-items: center; gap: 9px; }}
  .team-logo {{ width: 24px; height: 24px; object-fit: contain; flex-shrink: 0; }}
  .team-swatch {{ width: 24px; height: 24px; border-radius: 50%; flex-shrink: 0; background: var(--muted); }}
  .player {{ font-weight: 600; color: var(--white); }}
  .votes {{ font-weight: 700; color: var(--gold-bright); }}
  /* Predicted-vs-actual agreement marker in the player cell: a player present
     ...
     mirroring the model-vs-market divergence marker on the leaderboard page. */
  .agree {{ font-size: 11px; font-weight: 800; margin-right: 6px; cursor: help; vertical-align: middle; }}
...
<div class="wrap">
  ...
  <p class="subtitle">Real Brownlow Medal votes vs. the model's predicted leaderboard, for every player who reached either top 20.</p>
    ...
    <table>
      <thead>
        <tr>
          ...
          <th>Player</th>
          <th>Team</th>
          <th class="num">Actual votes</th>
          <th class="num">Predicted rank</th>
          <th class="num">Predicted votes</th>
        </tr>
      </thead>
      <tbody>
{rows}
...
_SEASON_REVIEW_ROW_TEMPLATE = (
    '        <tr>'
    '<td class="rank" style="border-left-color: {color};">{rank}</td>'
    '<td class="player">{agree}{player}</td>'
    '<td class="team-cell"><span class="team-inner">{logo}<span>{team}</span></span></td>'
    '<td class="votes num">{actual_votes:.1f}</td>'
    '<td class="num">{predicted_rank}</td>'
    '<td class="votes num">{predicted_votes:.1f}</td>'
    '</tr>'
)


def render_season_review(comparison: pd.DataFrame, hit_rate: float, output_path: str, season: int) -> None:
    """Render a static post-season predicted-vs-actual review page.
    ...
    produces. Rows are rendered in the order given (that function sorts real
    top-20 first by actual rank, then predicted-only players after). ``hit_rate``
    is the fraction (0-1) shown as a headline percentage.
    """
    rows = []
    for row in comparison.itertuples():
        ...
        if row.in_actual_top20 and row.in_predicted_top20:
            agree = '<span class="agree agree-hit" title="In both top 20s">&#10003;</span>'
        elif row.in_predicted_top20:
            title = "Predicted #{}, actually #{}".format(int(row.predicted_rank), int(row.actual_rank))
            agree = '<span class="agree agree-model-high" title="{title}">&#9650;</span>'.format(
                title=html.escape(title)
            )
        else:
            title = "Actually #{}, predicted #{}".format(int(row.actual_rank), int(row.predicted_rank))
            agree = '<span class="agree agree-model-low" title="{title}">&#9660;</span>'.format(
                title=html.escape(title)
            )

        rank_display = int(row.actual_rank) if row.in_actual_top20 else "—"
        rows.append(
            _SEASON_REVIEW_ROW_TEMPLATE.format(
                rank=rank_display,
                player=html.escape(str(row.player)),
                team=html.escape(str(row.team)),
                actual_votes=float(row.actual_votes),
                predicted_rank=int(row.predicted_rank),
                predicted_votes=float(row.predicted_season_votes),
                color=info["color"],
                logo=logo,
                agree=agree,
            )
