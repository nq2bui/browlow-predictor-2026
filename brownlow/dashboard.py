...
# --- Post-season predicted-vs-actual review page -----------------------------
#
# Unlike index.html/rounds.html (regenerated weekly, all season, by the
# production cron), this page only makes sense once the real Brownlow count is
# public -- afltables' round-by-round Brownlow page for a season doesn't exist
# until that season concludes. It's generated by a one-off manual script
# (backtest_2026.py), never by weekly_update.py.
#
# Layout deliberately mirrors the AFL's own official Live Tracker
# (afl.com.au/brownlow-medal/live-tracker): a round-by-round matrix where each
# cell stacks the REAL vote (bold, gold badge when >0) over the model's
# PREDICTED vote for that same round (small, muted), and the Total column does
# the same with season totals. Player headshots on the real tracker come from
# AFL's own licensed ChampID image CDN -- embedding those on a third-party site
# is a different call than scraping public match stats (closer to licensed
# media than data), so this uses the team logo we already have local, cleared
# rights to (brownlow/teams.py, same asset index.html/rounds.html use) instead
# of a hotlinked player photo. Decided 2026-09-22.

_SEASON_REVIEW_TEMPLATE = """<!doctype html>
<html>
<head>
<meta charset="utf-8">
<meta name="viewport" content="width=device-width, initial-scale=1.0">
<title>Brownlow Predictor {season} — Season Review</title>
<style>
  :root {{
    --grass: #0d1f0d;
    --panel: #12261a;
  ...
  * {{ box-sizing: border-box; }}
  ...
  .wrap {{ max-width: 1100px; margin: 0 auto; }}
  h1 {{ font-size: 30px; font-weight: 800; letter-spacing: -0.5px; margin: 0 0 4px; color: var(--white); }}
  h1 .accent {{ color: var(--gold); }}
  ...
  .headline .stat {{ font-size: 28px; font-weight: 800; color: var(--gold-bright); }}
  .headline .label {{ font-size: 12px; font-weight: 700; letter-spacing: 0.08em; text-transform: uppercase; color: var(--muted); }}
  ...
  thead th.round-col, thead th.total-col {{ text-align: center; }}
  tbody td {{
    padding: 8px 10px; border-bottom: 1px solid var(--row-line); font-size: 13px;
    font-variant-numeric: tabular-nums; text-align: center; white-space: nowrap;
  }}
  tbody tr:last-child td {{ border-bottom: none; }}
  tbody tr:hover td {{ background: rgba(240, 165, 0, 0.05); }}
  td.rank {{ color: var(--gold); font-weight: 800; width: 34px; }}
  td.player {{ text-align: left; font-weight: 600; color: var(--white); white-space: nowrap; }}
  .player-inner {{ display: flex; align-items: center; gap: 8px; }}
  .team-logo {{ width: 20px; height: 20px; object-fit: contain; flex-shrink: 0; }}
  .team-swatch {{ width: 20px; height: 20px; border-radius: 50%; flex-shrink: 0; background: var(--muted); }}
  /* Freeze the player-name column so it stays pinned while round columns
     scroll beside it -- same technique rounds.html uses. */
  thead th.player-name-col, tbody td.player {{ position: sticky; left: 0; border-right: 1px solid var(--line); }}
  tbody td.player {{ background: var(--panel); z-index: 1; }}
  thead th.player-name-col {{ background: var(--grass); z-index: 2; }}
  tbody tr:hover td.player {{ background: var(--panel); }}
  /* Each round cell stacks two numbers: the REAL vote (bold/gold when a vote
     was won) above the model's PREDICTED vote for that round (small, muted) --
     matching the AFL Live Tracker's own actual-over-predicted cell design. */
  td.cell {{ line-height: 1.15; }}
  td.cell .actual {{ display: block; }}
  td.cell .predicted {{ display: block; font-size: 10px; color: var(--muted); font-weight: 600; }}
  td.cell.v3 .actual {{ color: var(--gold-bright); font-weight: 800; }}
  td.cell.v3 {{ background: rgba(240, 165, 0, 0.12); }}
  td.cell.v2 .actual {{ color: var(--white); font-weight: 700; }}
  td.cell.v1 .actual {{ color: var(--white); font-weight: 600; }}
  td.cell.v0 .actual {{ color: var(--muted); }}
  td.cell.dnp {{ color: rgba(122, 154, 122, 0.4); }}
  td.total {{ font-weight: 800; border-left: 1px solid var(--line); line-height: 1.15; }}
  td.total .actual {{ display: block; color: var(--gold); }}
  td.total .predicted {{ display: block; font-size: 11px; color: var(--muted); font-weight: 700; }}
  thead th.total-col {{ border-left: 1px solid var(--line); }}
  /* Predicted-vs-actual agreement marker in the player cell: a player present
     in BOTH top-20s (a "hit") gets a plain check; a player in only ONE of the
     two top-20s gets an up/down arrow showing which side rated them higher. */
  .agree {{ font-size: 11px; font-weight: 800; margin-right: 4px; cursor: help; vertical-align: middle; }}
  .agree-hit {{ color: var(--gold-bright); }}
  .agree-model-high {{ color: var(--muted); }}
  .agree-model-low {{ color: var(--muted); }}
  .legend {{ display: flex; flex-wrap: wrap; gap: 16px; margin-top: 14px; color: var(--muted); font-size: 12px; }}
  .legend .swatch {{ font-weight: 800; }}
  .legend .swatch.v3 {{ color: var(--gold-bright); }}
  .legend .swatch.dnp {{ color: rgba(122, 154, 122, 0.55); }}
...
<body>
<div class="wrap">
  <h1>{season} Season <span class="accent">Review</span></h1>
  <p class="subtitle">Real Brownlow Medal votes (bold, gold) vs. the model's round-by-round prediction (small, muted), for every player who reached either top 20.</p>
  <p class="nav-link"><a href="index.html">&larr; Back to the live leaderboard</a></p>
  <div class="headline">
    <div class="stat">{hit_rate_pct}%</div>
    <div class="label">Top-20 hit rate</div>
  </div>
  <div class="card">
    <div class="matrix-scroll">
      <table>
...
{rows}
  ...
  <div class="legend">
    <span><span class="swatch v3">3</span> best on ground (real)</span>
    <span><span class="swatch">2 / 1</span> minor real votes</span>
    <span><span class="swatch dnp">&mdash;</span> did not play</span>
    <span><span class="agree agree-hit">&#10003;</span> in both top 20s</span>
    <span><span class="agree agree-model-high">&#9650;</span> model overrated</span>
    <span><span class="agree agree-model-low">&#9660;</span> model missed</span>
  </div>
...
_SEASON_REVIEW_CELL_TEMPLATE = '<td class="cell {cls}"><span class="actual">{actual}</span>{predicted}</td>'


def _season_review_agree_marker(row) -> str:
    if row.in_actual_top20 and row.in_predicted_top20:
        return '<span class="agree agree-hit" title="In both top 20s">&#10003;</span>'
    if row.in_predicted_top20:
        title = "Predicted #{}, actually #{}".format(int(row.predicted_rank), int(row.actual_rank))
        return '<span class="agree agree-model-high" title="{title}">&#9650;</span>'.format(
            title=html.escape(title)
        )
    title = "Actually #{}, predicted #{}".format(int(row.actual_rank), int(row.predicted_rank))
    return '<span class="agree agree-model-low" title="{title}">&#9660;</span>'.format(
        title=html.escape(title)
    )


def render_season_review(
    comparison: pd.DataFrame,
    actual_round_votes: pd.DataFrame,
    predicted_round_votes: pd.DataFrame,
    hit_rate: float,
    output_path: str,
    season: int,
) -> None:
    """Render a static post-season round-by-round predicted-vs-actual page.

    ``comparison`` is one row per player who reached either top 20 (real or
    predicted), with columns ``player, team, actual_rank, actual_votes,
    predicted_rank, predicted_season_votes, in_actual_top20,
    in_predicted_top20`` -- the shape ``backtest_2026.build_comparison_table``
    produces; rows render in the order given (real top-20 first by actual rank,
    then predicted-only players). ``actual_round_votes`` and
    ``predicted_round_votes`` are both ``player, round, votes`` frames (the same
    shape ``brownlow.weekly.per_round_votes`` returns) covering every round any
    of ``comparison``'s players played -- ``actual_round_votes`` from real
    scraped ``brownlow_votes``, ``predicted_round_votes`` from the model.
    ``hit_rate`` is the fraction (0-1) shown as a headline percentage.
    """
    players = [str(p) for p in comparison["player"]]
    all_rounds = sorted(
        {str(r) for r in actual_round_votes["round"]} | {str(r) for r in predicted_round_votes["round"]},
        key=round_sort_key,
    )
    actual_by_key = {
        (str(r.player), str(r.round)): float(r.votes) for r in actual_round_votes.itertuples()
    }
    predicted_by_key = {
        (str(r.player), str(r.round)): float(r.votes) for r in predicted_round_votes.itertuples()
    }

    header_cells = ['            <th class="player-col">#</th>']
    header_cells.append('            <th class="player-col player-name-col">Player</th>')
    for rnd in all_rounds:
        header_cells.append(
            '            <th class="round-col">{r}</th>'.format(r=html.escape(display_round_label(rnd, season)))
        )
    header_cells.append('            <th class="total-col">Total</th>')
    header_html = "\n".join(header_cells)
    ...
    for row in comparison.itertuples():
        ...
        code = info["code"]
        if code:
            logo = '<img class="team-logo" src="logos/{code}.png" alt="{team} logo">'.format(
                code=code, team=html.escape(str(row.team))
            )
        else:
            logo = '<span class="team-swatch"></span>'

        rank_display = int(row.actual_rank) if row.in_actual_top20 else "—"
        cells = [
            '<td class="rank">{rank}</td>'.format(rank=rank_display),
            '<td class="player"><span class="player-inner">{logo}<span>{agree}{name}</span></span></td>'.format(
                logo=logo, agree=_season_review_agree_marker(row), name=html.escape(player)
            ),
        ]
        for rnd in all_rounds:
            key = (player, rnd)
            if key not in actual_by_key and key not in predicted_by_key:
                cells.append('<td class="cell dnp">—</td>')
                continue
            actual_v = actual_by_key.get(key, 0.0)
            predicted_v = predicted_by_key.get(key, 0.0)
            predicted_html = (
                '<span class="predicted">{:g}</span>'.format(predicted_v) if predicted_v > 0 else ""
            )
            cells.append(
                _SEASON_REVIEW_CELL_TEMPLATE.format(
                    cls=_cell_class(actual_v),
                    actual="{:g}".format(actual_v) if actual_v > 0 else "",
                    predicted=predicted_html,
                )
            )
        cells.append(
            '<td class="total"><span class="actual">{:g}</span><span class="predicted">{:g}</span></td>'.format(
                float(row.actual_votes), float(row.predicted_season_votes)
            )
        )
        rows.append("          <tr>" + "".join(cells) + "</tr>")
    rows_html = "\n".join(rows)
    ...
    html_doc = _SEASON_REVIEW_TEMPLATE.format(
        season=season,
        hit_rate_pct="{:.0f}".format(hit_rate * 100),
        header=header_html,
        rows=rows_html,
        timestamp=timestamp,
    )
